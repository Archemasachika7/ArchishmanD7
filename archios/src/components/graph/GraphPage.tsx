...
export function GraphPage() {
  ...
  const groups = [...new Set(knowledgeNodes.map((n) => n.group))];
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      {/* Graph canvas */}
      <div
        className="os-panel relative"
        style={{ height: "520px", overflow: "hidden" }}
      >
        <KnowledgeGraph nodes={knowledgeNodes} edges={knowledgeEdges} filterGroup={filter} />

        {/* Legend */}
        <div
          className="absolute top-3 right-3 os-panel p-2 space-y-1"
          ...
          {groups.map((g) => {
            ...
            return (
              <div key={g} className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full" style={{ background: cfg.color }} />
                <span className="font-mono text-xs" style={{ color: "var(--text-muted)", fontSize: "0.6rem" }}>
                  {cfg.label}
                </span>
              </div>
            );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {knowledgeNodes
          ...
          .map((node) => {
            const cfg = groupLabels[node.group];
            return (
