...
import { Archive, FileText, Database, BookMarked, PenTool, Lock } from "lucide-react";
...
const typeConfig: Record<string, { icon: typeof FileText; color: string; label: string }> = {
  ...
  presentation: { icon: FileText, color: "#00ff88", label: "Presentation" },
};

export function VaultPage({ publicItems, totalCount }: { publicItems: VaultItem[]; totalCount: number }) {
  const privateCount = totalCount - publicItems.length;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold mb-1" style={{ color: "var(--text-primary)" }}>Engineering Vault</h1>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="os-panel p-3 text-center">
          <p className="font-mono text-2xl font-bold" style={{ color: "var(--accent-cyan)" }}>{totalCount}</p>
              ...
              return (
                <div key={item.id} className="os-panel p-4" style={{ borderColor: `${cfg.color}20` }}>
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-md flex items-center justify-center shrink-0 mt-0.5" style={{ background: `${cfg.color}15`, border: `1px solid ${cfg.color}30` }}>
                      <Icon size={14} style={{ color: cfg.color }} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <h3 className="font-semibold text-sm leading-tight" style={{ color: "var(--text-primary)" }}>{item.title}</h3>
                        <span className="font-mono text-xs px-1.5 py-0.5 rounded shrink-0" style={{ background: `${cfg.color}15`, color: cfg.color, fontSize: "0.6rem" }}>
                          {cfg.label}
                        </span>
                      </div>
      ...
      <div className="os-panel p-4 flex items-start gap-3" style={{ borderColor: "rgba(255,107,53,0.2)" }}>
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: "var(--text-secondary)" }}>
            CAD drawings, raw field notes, unreleased datasets, and draft research papers are kept private.
            Admin access required to review or publish.
          </p>
        </div>
      </div>
    </div>
